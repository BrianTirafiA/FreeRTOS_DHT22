...
static Display_State_e last_disp_state = DISP_STATE_END;
...
static Display_State_e disp_state = DISP_STATE_RAAAH;
...
static void Display_RAAAH( void );
...
static void Display_TemperatureHumidityChartAlternate( void );
...
void Display_Mng( void )
{
  ...
  last_disp_state = disp_state; // Store current display state
  switch( disp_state )
  {
    case DISP_STATE_RAAAH:
      Display_RAAAH();
      wait_time = millis();
      disp_state = DISP_STATE_RAAAH_WAIT;
      break;
    case DISP_STATE_RAAAH_WAIT:
    ...
    case DISP_STATE_TEMP_HUMID_SENSOR_REFRESH:
      ...
      break;
    case DISP_STATE_TEMP_HUMID_SENSOR_ALTERNATE:
      Display_TemperatureHumidityChartAlternate();
      disp_state = DISP_STATE_TEMP_HUMID_SENSOR_REFRESH;
      wait_time = millis();
      break;
    case DISP_STATE_END:
...
static void Touch_Read( lv_indev_drv_t * indev_driver, lv_indev_data_t * data )
{
  ...
  else
  {
    ...
    // Check for swipe event
        ...
        if (lastX != 0)
        {
            if (touchX - lastX > 50) // Swipe from left to right
            {
                // Go back to the RGB mixer screen
                if (last_disp_state == DISP_STATE_TEMP_HUMID_SENSOR ||
                    last_disp_state == DISP_STATE_TEMP_HUMID_SENSOR_REFRESH)
                {
                    disp_state = DISP_STATE_RGB_MIXER;
                }
            }
            else if (lastX - touchX > 50) // Swipe from right to left
            {
                // Go to the alternate chart display
                if (last_disp_state == DISP_STATE_TEMP_HUMID_SENSOR ||
                last_disp_state == DISP_STATE_TEMP_HUMID_SENSOR_REFRESH)
                {
                disp_state = DISP_STATE_TEMP_HUMID_SENSOR_ALTERNATE;
                }
                // Go back to the original chart display
                else if (last_disp_state == DISP_STATE_TEMP_HUMID_SENSOR_ALTERNATE)
                {
...
static void Display_RAAAH( void )
{
    ...
    lv_coord_t width = 0u;
    lv_coord_t height = 0u;

    lv_obj_t *Me_rectangle;

    lv_obj_t *act_scr = lv_scr_act(); // Get the active screen object

    Me_rectangle = lv_obj_create(act_scr);
    ...
    // created objects
    lv_obj_add_style(Me_rectangle, &slider_style, 0);

    width = lv_disp_get_hor_res(NULL);
    height = lv_disp_get_ver_res(NULL) / 2; // Half of the screen height

    lv_obj_set_size(Me_rectangle, width, height);
    lv_obj_align(Me_rectangle, LV_ALIGN_TOP_LEFT, 0, 0); // Align to top-left corner
    lv_obj_set_style_bg_color(Me_rectangle, lv_palette_main(LV_PALETTE_RED), LV_PART_MAIN); // Set background color to red
}
...
static void Display_TemperatureHumidityChart( void )
{
  ...
  lv_label_set_text( lbl_title, "(22/497916/TK/54592)");
  ...
  lv_style_set_line_color(&style_line_temp, lv_palette_main(LV_PALETTE_RED));
  
  static lv_style_t style_line_humid;
  lv_style_init(&style_line_humid);
  lv_style_set_line_width(&style_line_humid, 4);
  lv_style_set_line_color(&style_line_humid, lv_palette_main(LV_PALETTE_BLUE));
